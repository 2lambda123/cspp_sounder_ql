...
Cv_da = 719.0  # Specific heat at constant volume for dry air
Cp_v = 1870.0  # Specific heat at constant pressure for water vapour
Cv_v = 1410.0  # Specific heat at constant volume for water vapour
...
def svpwat_1(temp):
    ...
    s = a0 + t * (
        ...
        * (
            a2
            + t * (a3 + t * (a4 + t * (a5 + t * (a6 + t * (a7 + t * (a8 + t * a9))))))
        )
    )
    s = b / (s ** 8)
...
def dewpoint_AB(T_K, RH):
    ...
    gamma = log((RH / 100.0) * exp(exponent))
...
def Theta(tempk, pres, pref=100000.0):
...
def TempK(theta, pres, pref=100000.0):
...
def SatVap(tempc, phase="liquid"):
    ...
    over_liquid = 6.112 * exp(17.67 * tempc / (tempc + 243.12)) * 100.0
    over_ice = 6.112 * exp(22.46 * tempc / (tempc + 272.62)) * 100.0
...
def tsvwat(press):
    ...
    if vp > 0.1403e-4 and vp < 0.1233972e3:
        vp = log10(vp)
